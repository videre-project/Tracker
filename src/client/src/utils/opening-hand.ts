...
import type { GameLogDTO, GameLogType, ZoneTransferData } from "@/types/api"

const TYPE_ORDER: Record<GameLogType, number> = {
  GameState: 0,
  GameAction: 1,
  ZoneChange: 2,
  Reveal: 2,
...
export function getOpeningHandCards(
  logs: GameLogDTO[],
...
function isOpeningKeepAction(log: GameLogDTO) {
  ...
  try {
    const action = JSON.parse(log.data ?? "{}") as {
      name?: string
      response?: string
    }
